...
class StartEndDataset(Dataset):
    ...
    def _load_model_inputs(self, meta):
        ...
        if self.dset_name in ['tvsum']:
            ...
            if "relevant_windows" in meta: ## For Qvhighlights test set
                ...
                elif self.dset_name in ['hl']:
                    model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs["saliency_all_labels"] = \
                        self.get_saliency_labels_sub_as_query_multi(meta["relevant_windows"], meta["duration"], ctx_l, 2)  # multi gt
    ...
    def get_saliency_labels_sub_as_query_multi(self, gt_windows, duration, ctx_l, max_n=2):
        """
        Build positive/negative clip indices and the score array from multiple GT windows.

        Args:
            gt_windows: list of [start, end] time windows that represent GT regions.
            duration: video duration.
            ctx_l: number of clips the video is segmented into.
            max_n: number of positive/negative samples desired each time (default 2).

        Returns:
            pos_clip_indices: up to two positive clip indices (duplicated if only one candidate exists).
            neg_clip_indices: up to two negative clip indices (fallback to positive indices when insufficient).
            score_array: array of length ctx_l with ones inside GT regions and zeros elsewhere.
        """
        clip_len = duration / ctx_l
        score_array = np.zeros(ctx_l)

        # collect all GT clip indices into a set
        gt_clip_set = set()
        for gt_window in gt_windows:
            ...
            if gt_st > gt_ed:
                gt_st = gt_ed
            # mark GT regions in score_array
            score_array[gt_st:gt_ed + 1] = 1
            gt_clip_set.update(range(gt_st, gt_ed + 1))
        
        # convert the GT clip set to a list for sampling
        gt_clip_list = list(gt_clip_set)
        
        # handle cases where the GT clip pool is empty/small
        if len(gt_clip_list) == 0:
            pos_clip_indices = []
        elif len(gt_clip_list) == 1:
            pos_clip_indices = gt_clip_list * 2
        else:
            random.shuffle(gt_clip_list)
            pos_clip_indices = gt_clip_list[:max_n]

        # negatives come from clips outside GT regions
        neg_pool = [i for i in range(ctx_l) if i not in gt_clip_set]
        if len(neg_pool) < max_n:
            neg_clip_indices = pos_clip_indices  # not enough negatives; reuse positives
        else:
            neg_clip_indices = random.sample(neg_pool, k=max_n)
